...
def year_range(file, date_loc):
    max_year = 0
    min_year = 0
    with open(file, "r") as f:
        f.readline()
        min_year = int((((f.readline().strip("\n")).split(",")[date_loc]).split("-"))[0])
        for line in f:
            year = int((((line.strip("\n")).split(",")[date_loc]).split("-"))[0])
            if year < min_year:
                min_year = year
...
def load_price_data(file, descendingcsv, date_loc, opens_loc, closes_loc, starting_year, ending_year):
    dates = []
    opens = []
    closes = []
    start = False
    ...
    with open(file, "r") as f:
        f.readline()
        if descendingcsv == False:
            for line in f:
                line = line.split(",")
                date = (line[date_loc].split("-"))
