...
def plotting_dates(dates):
    """
    Build a plotting date list with no calendar gaps.
...
def graph_plot_values(dates,portfolio_values,delay):
    """
    Expand portfolio values to match calendar dates for plotting.

    `dates` only contains trading days, while the plotting x-axis includes
    weekends/holidays too. `delay` is the trading-day warmup before the first
    portfolio value exists (e.g. MA=30, RSI depends on lookback).
    """
    plting_dates = plotting_dates(dates)
    # Convert trading-day delay into the equivalent calendar-day index.
    offset = find_offset(plting_dates,dates,delay)
...
def find_offset(plting_dates,dates,delay):
    """
